...
public class ClientDaoImpl implements ClientDao {
    ...
    private static final String SQL_UPDATE_PAR_ID = "UPDATE Client SET nom = ?, prenom = ?, adresse = ?, telephone = ?, email = ?, image = ? WHERE id = ?";
    ...
    public void modifier( Client client ) throws DAOException {
        ...
        try {
            ...
            preparedStatement = initialisationRequetePreparee( connexion, SQL_UPDATE_PAR_ID, true,
                    client.getNom(), client.getPrenom(),
                    client.getAdresse(), client.getTelephone(),
                    client.getEmail(), client.getImage(), client.getId() );
            int statut = preparedStatement.executeUpdate();
            if ( statut == 0 ) {
                throw new DAOException( "Échec de la modification du client, aucune ligne ajoutée dans la table." );
            }
    ...
    private static Client map( ResultSet resultSet ) throws SQLException {
        ...
        try {
            ...
            Logger.getLogger(ClientDaoImpl.class.getName()).log(Level.SEVERE, null, ex.getMessage());
        }
